;;
;; This version uses subscription functions instead of relying on a registry.
;;
(ns space.matterandvoid.subscriptions.impl.eql-queries2
...
(defn eql-query-keys-by-type
  ...
  (let [set-keys          #(->> % (map :dispatch-key) set)
        ...
        plain-joins       (set (map (fn [j]
                                      [(:dispatch-key j) (join-prop->subscription (:dispatch-key j))])
                                 ;(juxt :dispatch-key (comp class->registry-key :component))
                                 plain-joins))
  ...
  (fn [db_ args]
    (make-reaction
      (fn []
        ...
        (let [refs        (not-empty (-attr datasource db_ id-attr join-prop args))
              ref-id-attr (-ref->attribute datasource refs)
              to-one?     (some? (-entity datasource db_ ref-id-attr (assoc args ref-id-attr (-ref->id datasource refs))))
              query       (get args query-key)]
          (cond
            to-one?
...
(defn create-component-subs
  ...
  [<sub datasource c join-subs-map]
...
(def comment-comp (nc {:query [:comment/id :comment/text {:comment/author (get-query user-comp)}]
                       :name  ::comment :ident :comment/id}))
(comment
  (create-component-subs <sub nil user-comp nil)
  (eql-query-keys-by-type (get-query user-comp) nil)
  (let [user-sub (create-component-subs <sub nil user-comp nil)]
    (eql-query-keys-by-type (get-query comment-comp) {:comment/author user-sub}))

  (let [user-sub (create-component-subs <sub nil user-comp nil)]
    (create-component-subs <sub nil comment-comp {:comment/author user-sub})
    )
