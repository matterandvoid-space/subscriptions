(ns space.matterandvoid.subscriptions.impl.eql4
  (:require
              ...
              (let [entity           (get-in @db_ [id-attr entity-id])
                ...
                (cond
                  ...
                  (do
                    ...
                    (println "sub: " [join-component-sub (apply assoc args rels)])
                    (let [[kw id] rels]
                      (<sub db_ [(join-component-sub kw) (assoc args kw id ::subs/query (union-branch-map kw))])))
...
(comment
  ...
  (<sub db_ [::list {:list/id 1 ::subs/query [:list/name {:list/members {:comment/id [:comment/id :comment/text {:comment/sub-comments '...}]
...
(defn reg-sub-recur-join
  ...
  (reg-sub-raw recur-prop
    (fn [db_ {::subs/keys [parent-query] :as args}]
      (make-reaction
        (fn []
